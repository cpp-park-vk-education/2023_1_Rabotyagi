class MessageTable : DatabaseTable
{
    ...
    bool Checker(unsigned int owner_id, unsigned int channel_id){
    ...
    bool updateRecord(Message& message_struct, unsigned int id) override
    {
        try{
            pqxx::work transaction(*conn);
    ...
    Message readRecord(unsigned int id)
    {
        try{
            pqxx::work transaction(*conn);
